...
function run() {
    ...
    //         MESSAGE.subscribe(M => SlackSendAlert(M));
    //     } else {
    //         throw new Error("Unable to send test message to Slack app. Check webhook URI");
    //     }
    // })
    ...
    //     ZertoLogin(zvm, (token, msg) => {
    //         if(!token) {
    //             throw new Error("Authentication to Zerto ZVM " + zvm["address"] + " failed " + msg);
    //         }
    ...
    //             let zLabel = zvm["label"] || zvm["address"];

    //             logger.info("Getting alerts from " + zLabel);
                
    //             ZertoGetAlerts(zvm, (msg, data) => {
    //                 if(msg.code == 200) {                
    //                     Array.from(data).forEach(alert => {
    //                         // verify alert is not in cache
    //                         if(IsAlertNew(alert)) {
    //                             logger.info("The alert is new " + alert["Link"]["identifier"]);
    ...
    //                             MESSAGE.next(M);
    //                         } else {
    //                             logger.info("Alert is already in cache, skipping " + alert["Link"]["identifier"]);
    //                         }
    //                     })
    
    //                     // cleanup aknowledged alerts
    //                     RemoveStaleAlerts(data);
...
function ZertoGetAlerts(zerto, callback) {
    let zPort = zerto["port"] || 9669;
    let zHost = zerto["address"] || null;
    let zToken = zerto["token"] || null;
...
function SlackSendAlert(message) {
    ...
    // let zerto = message["zerto"];

    // let zLabel = zerto ? zerto["label"] || zerto["address"] : "N/A";
